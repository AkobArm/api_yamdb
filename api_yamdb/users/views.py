...
from .serializers import (UserSerializer, UserSignUpSerializer, UserSelfSerializer)
...
def sign_up(request):
    ...
    if user.exists():
        ...
        generate_token(user)
        return Response(
            {'message': 'Пользователь с таким email уже существует. Код подтверждения отправлен повторно.'},
